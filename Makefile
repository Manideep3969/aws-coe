...
test: test-python test-scp test-terraform ## Run all tests

test-python: ## Run Python unit tests
	@echo "$(GREEN)Running Python unit tests...$(NC)"
	python3 -m pytest tests/unit/ -v --tb=short

test-python-cov: ## Run Python unit tests with coverage
	@echo "$(GREEN)Running Python unit tests with coverage...$(NC)"
	python3 -m pytest tests/unit/ -v --tb=short --cov=incident-response/automation/src --cov-report=term-missing

test-scp: ## Validate SCP JSON policies
	@echo "$(GREEN)Validating SCP policies...$(NC)"
	@python3 -c "import json, os, sys; \
		errors = 0; \
		policies_dir = 'scp/policies'; \
		[\
			(print(f'  OK: {f} - {len(json.load(open(os.path.join(policies_dir, f))).get(\"Statement\", []))} statements') if json.load(open(os.path.join(policies_dir, f))).get('Version') == '2012-10-17' and 'Statement' in json.load(open(os.path.join(policies_dir, f))) else (print(f'  ERROR: {f}'), errors := errors + 1)) \
			for f in os.listdir(policies_dir) if f.endswith('.json')\
		]; \
		sys.exit(errors)"

test-lambda: ## Run Lambda function unit tests only
	@echo "$(GREEN)Running Lambda unit tests...$(NC)"
	python3 -m pytest tests/unit/test_isolate_ec2.py tests/unit/test_disable_iam_key.py -v --tb=short

test-terraform: ## Validate Terraform structure and conventions
	@echo "$(GREEN)Running Terraform structure tests...$(NC)"
	python3 -m pytest tests/unit/test_terraform_structure.py -v --tb=short

test-security: ## Run security scans (bandit + tfsec if available)
	@echo "$(GREEN)Running security scans...$(NC)"
	@python3 -m bandit -c -ll -r incident-response/automation/src/ 2>/dev/null || echo "$(YELLOW)Bandit not available, skipping$(NC)"
	@tfsec . 2>/dev/null || echo "$(YELLOW)TFSec not available, skipping$(NC)"

test-all: test-python test-scp test-terraform test-security ## Run all tests including security scans

pre-commit: ## Run all pre-commit checks (fmt + validate + tests)
	@echo "$(GREEN)Running pre-commit checks...$(NC)"
	@bash scripts/run-tests.sh

install-hooks: ## Install git pre-commit hooks
	@echo "$(GREEN)Installing git pre-commit hooks...$(NC)"
